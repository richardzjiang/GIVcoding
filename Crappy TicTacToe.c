...
/*
if (linehighestrisk == 1)




//remember, if incentive is already 0, then incentive can no longer be increased, because if incentive is 0, then it means that that slot should not be played by the computer.
//code to calculate slot with highest incentive.

if (equalrisk > 1) {
    srand(time(NULL));
    random = rand() % equalrisk + 1;
    equalriskstring[random]
    //a random number between 1 and 3 (inclusive) is generated. This random number is used to decide which 
    
    //if highestrisk is 2, then the remaining slot receives a very high incentive.
} else { //no lines are equal to each other
